...
<script src="./resources/verify.js"></script>
...
<script>
	function submitForm(event) {
		...
		var SocialSecurityNumber = document
				.getElementById('SocialSecurityNumber').value;
		var Year = document.getElementById('Year').value;
		var Month = document.getElementById('Month').value;
		var Day = document.getElementById('Day').value;

		var interElements = document.getElementsByName('inter');
		var interValues = [];
		for (var i = 0; i < interElements.length; i++) {
			if (interElements[i].checked) {
				interValues.push(interElements[i].value);
			}
		}
		var inter = interValues.join(',');
		var Intro = document.getElementById('Intro').value;

		var formAction = './Join4/' + ID + '/' + PW + '/' + PWconfirm + '/'
				+ Email + '/' + Name + '/' + SocialSecurityNumber + '/' + Year
				+ '/' + Month + '/' + Day + '/' + inter + '/' + Intro;
...
<body>
	...
	<br>
	<hr>
	<!-- onsubmit="submitForm(event)" pathvariable 사용할때 -->
	<form id="join" action="./Join2" method=post >
		...
		<p style="text-align: center;">
			<button onclick="submitForm(event)">pathvariable버튼</button>

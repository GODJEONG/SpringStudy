    ...
    */
    ...
    var updateSelectedOptions = function (type) {
        ...
        if (require_option_count == 0) {
            let section_total_price = order_count * prod_price;   // 본 상품 가격 총합
        ...
        $.each(selected_options, function (no, data) {
            ...
            if (data.require == true) {
                ...
                if (option_period_discount_price > option_price) option_period_discount_price = option_price;
				...
				if(is_use_prod_additional){
					...
					$.each(selected_prod_additionals, function (no, data) {
						...
						if (prod_additional_require_option_count_by_data_idx == 0) {
							...
							prod_additional_total_price = parseInt(data.count) * parseInt(data.price);
							...
							period_discount_price += prod_additional_period_discount_price;
					...
					$.each(selected_prod_additional_options, function (no, data) {
						...
						if (data.require) {
							...
							if (option_period_discount_price > option_price) option_period_discount_price = option_price;
							period_discount_price += option_period_discount_price;
							if (prod_data.option_mix_type == 'MIX' || prod_additional_period_dc_type[data.prod_idx] == 'percent') {
								show_period_discount = true;
								option_price = option_price - option_period_discount_price;
							}
						}
     ...
     */
    var _addOrder = function (type, backurl, is_c3, callback){
    ...
    var addOrder = function (type, backurl, params) {
        if (add_order_progress_check) return false;
            ...
            _addOrder(type, backurl, false, function(result){
                switch (type) {
                    case 'npay':
                        if (result.npay_url == '') {
                            if (result.errmsg) {
    ...
    var OMS_addOrder = function (type, backurl, params) {
        if(add_order_progress_check) return false;
			...
			_addOrder(type, backurl, true, function(result){
				switch( type ) {
					case 'npay':
						if(result.npay_url == '') {
							if ( result.errmsg ) {
						...
						switch ( params.type ) {
							...
							case 'onPayOrder':
								if ( result.msg == 'SUCCESS' ) {
									...
									if ( typeof CHANNEL_PLUGIN != 'undefined' ) CHANNEL_PLUGIN.AddOrder(result.order_code);
									if (typeof NP_NDA != 'undefined') NP_NDA.BeginCheckout(result.prod_code_list);
									window.location.href = "/shop_payment/?order_code=" + encodeURIComponent(result.order_code) + '&order_no=' + encodeURIComponent(result.order_no);
								}
								break;
						};
    ...
    var showMobileOptions = function (type) {
		if (!options.is_price_view_permission && window.IS_GUEST) {
			...
			if (/^Y$/i.test(cm_data.use_login_popup)) {
				window.SITE_MEMBER.openLogin(backUrl,'payment',function(){
					window.SITE_SHOP_DETAIL.confirmOrderWithCartItems('normal', window.location.href);
				}, 'N', 'payment');
				return;
			} else {
    ...
    var confirmOrderWithCartItems = function (type, backurl, params) {
        ...
        var isOrderTypeRegularly = $('input[name=add_cart_type]:checked').val() === 'regularly' ;
        // 단일 상품 주문할 경우

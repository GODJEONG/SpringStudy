...
public class LoginDAO {
    ...
    public boolean membercheck(String id, String pw) {
        System.out.println("check: " + template.toString());
        System.out.println(id +"/"+ pw);
        String query = "SELECT count(id) as cnt FROM shop_member WHERE id ='" +id + "' AND pw = '" + pw + "'";
        
        int count = 0;
        count = this.template.queryForObject(query,Integer.class);

        System.out.println("멤버 결과: " + count);
